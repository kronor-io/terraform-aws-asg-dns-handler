    ...
    if ec2_response["State"]["Name"] == "running":
        if "use_public_ip" in os.environ and os.environ["use_public_ip"] == "true":
            try:
                ...
                logger.info(
                    "Found public IP for instance-id %s: %s", instance_id, ip_address
                )
            except:
                logger.info(
                    "No public IP for instance-id %s: %s", instance_id, ip_address
                )
        else:
            try:
                ...
                logger.info(
                    "No private IP for instance-id %s: %s", instance_id, ip_address
                )
...
def update_record(zone_id, ips, hostname):
    if len(ips) == 0:
        ...
        operation = "UPSERT"
    logger.info(
        "Changing record with %s for %s -> %s in %s", operation, hostname, ips, zone_id
    )
...
def process_asg(auto_scaling_group_name, hostname, ignore_instance):
    ...
    if ignore_instance is None:
        ...
        logger.info(
            "Ignoring instance-id %s while Processing ASG %s",
            ignore_instance,
            auto_scaling_group_name,
        )
    for instance in autoscaling.describe_auto_scaling_groups(
...
def process_message(message):
    ...
    if message["LifecycleTransition"] not in (
